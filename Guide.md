...
> 対応版: v1.3.x 系 / current public repository UI。
> このガイドは、単一起動、構造タブ、原子対ボンド長編集、配置プレビュー、supercell / 真空層 / セル軸傾き、原子一覧 PNG、前駆体配置、吸着分子ポーズ編集まで含む新しい操作版です。まず全体像を掴みたい場合は、画像付きの `README.md` から読むと早いです。
...
9. `保存` で `.aseproj/.json`、CIF、POSCAR、XYZ / extended XYZ 形式に保存する。
...
| 方法 | 操作 |
| --- | --- |
| ツールバー | `開く` を押してファイルを選ぶ。ショートカットは `Ctrl+O`。既存構造がある場合は新しいタブに読み込む。 |
...
| `.cif` | 編集後の現在構造を P1 の CIF として保存。全原子とセルを `_atom_site_fract_*` / `_atom_site_Cartn_*` に書き出す。 |
...
保存は `保存` または `Ctrl+Shift+S` です。保存前に構造診断が表示されるため、原子数やセル状態を確認してから保存できます。CIF は現在の編集結果を P1 として書き出します。extended XYZ は ASE 互換のため Cartesian `pos` として保存します。
...
| 領域 | 役割 |
| --- | --- |
| 中央キャンバス | 構造を球・均一太さの両端原子色ボンド・セル枠で表示し、選択・視点移動・原子移動を行う。 |
...
| `開く` | 構造ファイルを新しいタブへ読み込む。未編集の初期タブだけは置き換える。 |
| `保存` | 編集中の構造を保存する。 |
...
| `セル` / `ボンド` / `格子外` / `軸` / `ラベル` | ツールバーまたは `表示` メニューからオン/オフを切り替える。`格子外` をオンにすると、VESTA風に周期境界で単位格子外へ続く複製原子とボンドを表示し、オフにすると単位格子内だけに戻す。 |
| `原子対ボンド長` | 読み込まれている構造の全原子対距離を一覧し、Target Å を個別に修正する。 |
| `透視投影` | 遠近感のある投影に切り替える。 |
| `奥行き` | 奥の原子を薄く見せる奥行き表現を切り替える。 |

### 原子対ボンド長

`原子対ボンド長` では、読み込まれている構造の全原子対を距離順に一覧できます。各行には Atom A、Atom B、現在距離、Target Å が表示されます。Target Å を変更し、「変更済みをすべて適用」または「選択行を適用」を押すと、Atom A を固定したまま Atom B が現在の原子対方向に沿って移動します。

`元素ペアボンド長` では、`Ga-N` など表示ボンドとして判定されている同じ元素ペアを選び、Target Å を1回入力して一括変更できます。`動かす元素` を `N` にすると Ga 側を固定し、N 側を各 Ga-N ボンド方向へまとめて移動します。結晶内で1つの N が複数の Ga と結合している場合は、すべての指定ボンドに対する平均位置へ移動し、適用後の最大誤差がステータスバーに表示されます。
...
回転セルや表面再構成用セルが必要な場合は、`格子変換行列を直接入力する` をオンにして、a'/b'/c' を元の a/b/c の整数係数で入力します。
例: 正方格子の `√2×√2 R45°` は `a' = a + b`, `b' = -a + b`, `c' = c` なので、行列は `[1 1 0; -1 1 0; 0 0 1]` です。
...
| 特定原子対の距離を直したい | `原子対ボンド長` で対象行の Target Å を変更して適用する。 |
| 保存形式で迷う | 後で再編集するなら `.aseproj`、結晶構造の受け渡しなら `.cif`、計算投入なら POSCAR 系、ASE に cell / PBC 付きで戻すなら `.extxyz` を選ぶ。 |
