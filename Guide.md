...
> このガイドは、単一起動、構造タブ、ボンド表示最大距離編集、配置プレビュー、supercell / 真空層 / セル軸傾き、原子一覧 PNG、前駆体配置、吸着分子ポーズ編集まで含む新しい操作版です。まず全体像を掴みたい場合は、画像付きの `README.md` から読むと早いです。
...
| `ボンド表示最大距離` | Ga-N、Ga-H、N-H など原子種ペアを1行ずつ一覧し、表示するボンドの最大距離だけを変更する。 |
| `透視投影` | 遠近感のある投影に切り替える。 |
| `奥行き` | 奥の原子を薄く見せる奥行き表現を切り替える。 |

### ボンド表示最大距離

`ボンド表示最大距離` では、`Ga-N`、`Ga-H`、`N-H` など原子種ペアを1行ずつ一覧し、ボンドとして表示する最大距離だけを変更できます。これは表示設定であり、原子座標、保存される構造、Undo/Redo 履歴は変更しません。例えば Ga-N の最大距離を短くすると、その距離を超える Ga-N ボンド線だけが非表示になります。
...
| 表示されるボンドの範囲を変えたい | `ボンド表示最大距離` で Ga-N、Ga-H、N-H などの最大表示距離を変更する。 |
