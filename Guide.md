...
このガイドは、ASEapp Surface Builder で構造ファイルを読み込み、表面 slab / supercell / 真空層 / 吸着原子 / 前駆体配置を作るための実用手順です。スクリーンショットは `guide_assets/sample_GaN.cif` を読み込んだ Windows 版の例です。
...
6. 必要な時だけ `配置プレビューを表示` をオンにして予定位置を確認する。
7. `配置する` で原子または前駆体を追加する。
8. `保存` で `.aseproj/.json` または POSCAR 形式に保存する。
...
| 右パネル `2. スーパーセル/真空` | supercell、真空層、真空層除去、セル軸傾きを操作する。 |
| 右パネル `3. 視点操作` | direct/reciprocal 視点、操作モード、フィット、direct c リセットを操作する。 |
...
3. 必要なら `周期表...` から元素を選ぶ。
4. `配置位置` を選ぶ。
5. `高さ/距離 Å`、`傾き角度 °`、必要に応じて `2原子間の比率` を調整する。
...
| `2原子の間` | 2 個 | 2 原子の中点に配置する。 |
| `2原子の間・比率指定` | 2 個 | `2原子間の比率` で位置を指定する。`0=1個目`、`1=2個目`。 |
| `3原子の中心` | 3 個 | 3 原子の重心に配置する。hollow site の確認に便利。 |
| `選択原子すべての中心` | 1 個以上 | 選択中の全原子の中心に配置する。 |
| `選択面の法線上` | 3 個以上 | 選択原子で作る面の法線方向に、指定距離だけ離して配置する。 |

`高さ/距離 Å` は、基準点から法線方向へ動かす距離です。`0` にすると、原子間/中心そのものへ配置します。`傾き角度 °` は法線方向からの傾きで、2 個以上選択している場合は 1 個目から 2 個目の方向へ傾きます。
...
1. 隣り合う表面原子を 2 個選択する。
2. `配置位置` を `2原子の間` または `2原子の間・比率指定` にする。
3. 中点なら比率 `0.5`、偏らせたい場合は `0-1` で調整する。
4. `配置する`。
...
| プレビューが見えない | `配置プレビューを表示` がオンか確認する。通常はオフ。 |
...
| 保存形式で迷う | 後で再編集するなら `.aseproj`、計算投入なら POSCAR 系を選ぶ。 |
