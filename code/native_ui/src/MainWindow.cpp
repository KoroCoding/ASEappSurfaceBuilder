...
public:
    explicit UsageHelpDialog(bool japanese, QWidget* parent = nullptr) : QDialog(parent) {
        ...
        label->setText(japanese
            ...
            "5. Supercell で a/b/c 方向に拡張します。\n"
            "6. セル軸傾きで、c軸などをa/b方向へ傾けてステップテラス候補を作れます。\n"
            "7. 真空層 で真空層の追加・除去、またはスラブ全体の移動を行います。真空層除去ボタンでは c軸方向をすぐ詰められます。\n"
            "8. 原子一覧PNG で、論文・学会用の球＋ラベル画像を解像度/DPI指定で出力します。\n\n"
            "視点操作:\n"
            "・初期視点: c 方向\n"
            "・左ドラッグ: 回転\n"
            ...
            "3. Choose a placement option and Apply to place atoms on top / between atoms / multi-point centers. Directly above/below applies to every selected atom.\n"
            ...
            "5. Use Supercell to repeat along a/b/c.\n"
            "6. Use Axis tilt to tilt c or another cell axis toward a/b for step-terrace candidates.\n"
            "7. Use Vacuum to add/remove vacuum or move the whole slab. Remove vacuum immediately tightens the c-axis.\n"
            "8. Use Atom legend PNG to export a sphere-and-label image for papers or presentations with selectable resolution/DPI.\n\n"
            "View controls:\n"
            "・Initial view: c-axis\n"
            "・Left drag: rotate\n"
...
public:
    explicit StartupGuideDialog(bool japanese, QWidget* parent = nullptr) : QDialog(parent) {
        ...
        body->setText(japanese ? QStringLiteral(
            ...
            "<li>Choose a placement option, then Apply to place atoms directly above/below, between atoms, or at multi-point centers. Direct above/below applies to all selected atoms.</li>"
...
QString MainWindow::uiText(const QString& key) const {
    if (m_japanese) {
        ...
        if (key == "supercell_group") return QStringLiteral("2. スーパーセル/真空");
        if (key == "view_group") return QStringLiteral("3. 視点操作");
        ...
        if (key == "mode_n_center") return QStringLiteral("選択原子すべての中心");
        ...
        if (key == "supercell_group") return QStringLiteral("2. Supercell/Vacuum");
        if (key == "view_group") return QStringLiteral("3. View controls");
        ...
        if (key == "mode_n_center") return QStringLiteral("All selected center");
...
void MainWindow::buildUi() {
    ...
    m_placementModeCombo->addItem(uiText(QStringLiteral("mode_between")), QStringLiteral("pair_midpoint"));
    m_placementModeCombo->addItem(uiText(QStringLiteral("mode_between_fraction")), QStringLiteral("pair_fraction"));
    m_placementModeCombo->addItem(uiText(QStringLiteral("mode_three_center")), QStringLiteral("triple_centroid"));
    m_placementModeCombo->addItem(uiText(QStringLiteral("mode_n_center")), QStringLiteral("multi_centroid"));
    ...
    connect(m_placementModeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this]() {
        if (m_placementHeightSpin != nullptr && m_placementModeCombo != nullptr) {
            ...
            if (mode == QStringLiteral("pair_midpoint")
...
void MainWindow::openStructure() {
    const QString path = QFileDialog::getOpenFileName(this, "Open structure", defaultOpenDirectory(),
        "Structure files (*.aseproj *.json *.cif *.xyz *.vasp POSCAR CONTCAR *.poscar *.pdb *.xsf);;All files (*.*)");
...
void MainWindow::saveStructureAs() {
    const QString path = QFileDialog::getSaveFileName(this, "Save structure", defaultOpenDirectory() + "/" +
        ...
        "ASEapp project (*.aseproj *.json);;VASP POSCAR (*.vasp POSCAR CONTCAR *.poscar)");
...
void MainWindow::pushUndoState(const QString& label) {
    m_undoStack.push_back(m_structure);
...
void MainWindow::undoEdit() {
    ...
    m_redoStack.push_back(m_structure);
    m_structure = m_undoStack.back();
...
void MainWindow::refreshPresetUi() {
    ...
    if (m_canvas != nullptr) {
        ...
        const bool showPreview = m_previewPlacementCheck != nullptr && m_previewPlacementCheck->isChecked();
        if (showPreview && static_cast<int>(m_selectedAtomIds.size()) >= required) {
            QString errorMessage;
...
QString MainWindow::describePlacementRule(const SurfacePlacementRule& rule) const {
    ...
    else if (mode == "pair_midpoint") lines << QStringLiteral("Placement: midpoint between two selected atoms");
    else if (mode == "pair_fraction") lines << QStringLiteral("Placement: line interpolation between two selected atoms");
    else if (mode == "triple_centroid") lines << QStringLiteral("Placement: centroid of three selected atoms");
    else if (mode == "triple_weighted") lines << QStringLiteral("Placement: weighted centroid of three selected atoms");
    else if (mode == "multi_centroid") lines << QStringLiteral("Placement: centroid of all selected atoms");
    else if (mode == "multi_weighted") lines << QStringLiteral("Placement: weighted centroid of all selected atoms");
